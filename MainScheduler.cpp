...
void age_FCFS(vector<queue<Process*> >& vector_queues, int queue_index, int queue_size, int age_interval, fstream& fs){
    while(queue_size > 0){// go through the queue to age
        Process* proc_front = vector_queues[queue_index].front(); //grab the last queue from all the queues
        ...
        if(proc_front->age >= age_interval){ //age it up to the next queue
            ...
            fs << "Process " << proc_front->pid << " has aged and was promoted to queue " << queue_index-1 << "\n";
        }else{
...
void MFQS(vector<Process*> processes, fstream& fs){
    ...
    vector_queues.push_back(queue5);
    
    Process* cpu_process = NULL;
    
    int MFQS_timer = 0;
    const int master_time_quantum = 1;
    ...
    while( containProcesses(processes, vector_queues) ) {
        ...
        if(tqexpired || (cpu_process != NULL && cpu_process->burst==0)){//we are ready to grab a new process
            ...
            if(cpu_process != NULL && cpu_process->burst==0){
                master_completed++;
                cpu_process->waitingTime = (MFQS_timer - cpu_process->waitingTime) - cpu_process->master_burst;
...
//                fs << "Process " << cpu_process->pid << " ran from " << MFQS_timer - (master_queue_time_quantum - time_quantum) << " through " << MFQS_timer-1 << " in queue " << queue_index << " and has finished\n";
            ...
            if(queue_index < vector_queues.size()){// need to check if we actually found a queue with processes or we ran over
                ...
                if(cpu_process->master_burst == cpu_process->burst){
                    cpu_process->waitingTime = MFQS_timer;
                }
        ...
        if(queue_index+1 == vector_queues.size()){ //if we are in the last one, dont do time quantum
            ...
            //remove the first process, because we shouldn't update this processes age timer
            ...
            age_FCFS(vector_queues, queue_index, queue_size, age_interval, fs);//age everything in the last queue
            ...
            age_FCFS(vector_queues, vector_queues.size()-1, queue_size, age_interval, fs);//age everything in the last queue
...
//                    fs << "Process " << proc->pid << " ran from " << MFQS_timer-master_queue_time_quantum+1 << " through " << MFQS_timer << " in queue " << queue_index << " and has been demoted to queue " << queue_index+1 << "\n";
